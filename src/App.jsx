...
function groupMatches(slotGroup, baseGroup, excludedSubs = []) {
  ...
  const ns = ngs(slotGroup), nb = ngs(baseGroup);
...
function slotsInRange(start, end) {
  const si = MORNING_SLOTS.indexOf(start), ei = MORNING_SLOTS.indexOf(end);
...
function analyzeTeacher(teacher, trip) {
  const {
    day, startSlot, endSlot, selectedGroups, excludedSubs, halfGroups, subject,
  } = trip;
  ...
  if (subject)
    Object.values(teacher.schedule).forEach((slots) =>
      slots.forEach((s) => {
        if (s.type === "class" && s.subject.toLowerCase().includes(subject.toLowerCase()))
          subjectMatch = true;
      })
    );
  ...
  const score = Math.max(
    0,
    rangeCoverageScore + groupTeacherBonus + coveredSlots * 20 +
      (subjectMatch ? 15 : 0) + morningOccupied * 3 + freeCount * 1 -
      stayClasses.length * 12 - halfClasses.length * 6
  );
  const label = spanLabel(span, daySchedule);

  return {
    name: teacher.name, score, coveredSlots, totalRange: totalRangeSlots,
    coversFullRange, hoursShort, teachesGroup, spanLabel: label,
    tripClasses, stayClasses, halfClasses, guardSlots, freeCount,
    morningOccupied, subjectMatch, daySchedule, inRange, span,
  };
}
    ...
    teacherSlots.forEach((time) => {
      ...
      if (leavesGap) {
        gapsBySlot[time].push({
          teacherName: teacher.name, subject: slot.subject,
          group: slot.group, room: slot.room,
...
function Spinner() {
  return (
    <div style={{
      ...
      border: "3px solid #e5e7eb", borderTopColor: "#e8451e",
...
function Avatar({ name, color }) {
  ...
  return (
    <div style={{
      ...
      background: color || "#1a2744", color: "white",
...
function Pill({ color, children, small }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: 99, backgroundColor: c.bg, color: c.fg,
...
function Card({ title, hint, children, accent }) {
  return (
    <div style={{
      ...
      border: `1px solid ${accent ? "#e8451e" : "#e5e7eb"}`, marginBottom: 12,
    }}>
      {title && (
        <p style={{
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: "#6b7280", minWidth: 48 }}>Inici</span>
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          {MORNING_SLOTS.map((slot, i) => {
            const a = i === si, d = i > ei;
            return (
              <button key={slot} disabled={d} onClick={() => onChange(slot, endSlot)}
                style={{
                  padding: "5px 10px", borderRadius: 6,
                  border: `1.5px solid ${a ? "#1a2744" : "#e5e7eb"}`,
                  backgroundColor: a ? "#1a2744" : "white",
                  color: a ? "white" : d ? "#d1d5db" : "#374151",
                  fontSize: 12, fontWeight: 500, cursor: d ? "not-allowed" : "pointer",
                }}>{slot.split("-")[0]}</button>
            );
          })}
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: "#6b7280", minWidth: 48 }}>Final</span>
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          {MORNING_SLOTS.map((slot, i) => {
            const a = i === ei, d = i < si;
            return (
              <button key={slot} disabled={d} onClick={() => onChange(startSlot, slot)}
                style={{
                  padding: "5px 10px", borderRadius: 6,
...
function GroupSelector({ selected, onChange, teachers, excludedSubs, onExcludedSubs, halfGroups, onHalfGroups }) {
  const subgroupsPerGroup = useMemo(() => {
    const m = {};
    selected.forEach((g) => { m[g] = detectSubgroups(teachers || [], g); });
    return m;
  }, [selected, teachers]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: subs.length > 0 ? 8 : 0 }}>
                  ...
                  {subs.length === 0 && (
                    <button onClick={() => toggleHalf(g)}
                      ...
                      }}>{isHalf ? "½ Mig grup va de sortida" : "Tot el grup va de sortida"}</button>
                  )}
                </div>
                {subs.length > 0 && (
                  <>
                    <p style={{ fontSize: 11, color: "#854d0e", fontWeight: 600, margin: "0 0 6px" }}>⚠ Grups partits — indica quins van:</p>
                    <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                      {subs.map((sub) => {
                        ...
                        return (
                          <button key={sub} onClick={() => toggleSub(sub)}
                            style={{
                              ...
                              border: `1.5px solid ${goes ? "#166534" : "#9ca3af"}`,
      ...
      {selected.length > 0 && (
        <button onClick={() => { onChange([]); onExcludedSubs([]); onHalfGroups([]); }}
          style={{ alignSelf: "flex-start", background: "none", border: "none", color: "#9ca3af", fontSize: 12, cursor: "pointer", textDecoration: "underline", marginTop: 2 }}>
          Netejar selecció
        </button>
      )}
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {subjects.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {subjects.map((s) => {
            const a = selected === s;
            return (
              <button key={s} onClick={() => onChange(a ? "" : s)}
                style={{
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <input
          style={{ flex: 1, padding: "8px 12px", border: "1.5px solid #e5e7eb", borderRadius: 8, fontSize: 13, color: "#1a1a2e" }}
...
function CoveragePanel({ teachers, confirmedNames, trip, franges, teacherFranjaMap, assignments, onAssign }) {
  ...
  if (!hasAnyGap)
    return (
      <div style={{ padding: "14px 18px", backgroundColor: "#f0fdf4", borderRadius: 10, border: "1px solid #bbf7d0" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {rangeSlots.map((time) => {
        ...
        const covers = coversBySlot[time] || { freed: [], guardL: [], guardE: [], guardA: [], guardB: [], free: [] };
        if (!gaps.length)
          return (
            <div key={time} style={{
              padding: "8px 14px", backgroundColor: "#f0fdf4", borderRadius: 8,
        ...
        const totalAvail = covers.freed.length + covers.guardA.length + covers.guardB.length;
        ...
        return (
          <div key={time} style={{ borderRadius: 10, border: `1.5px solid ${borderColor}`, backgroundColor: bgColor, overflow: "hidden" }}>
            <div style={{
              ...
              <span style={{ fontSize: 11, opacity: 0.65 }}>{time}</span>
              <span style={{ marginLeft: "auto", fontSize: 11, fontWeight: 700, color: allAssigned ? "#166534" : "#991b1b" }}>
                {allAssigned ? "✓ Cobert" : `⚠ ${gaps.filter((_, gi) => !assignments[`${time}|${gi}`]).length} forat${gaps.length > 1 ? "s" : ""} per cobrir`}
              </span>
            </div>
            <div style={{ padding: "12px 14px", display: "flex", gap: 16, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 200px" }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: "#991b1b", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 8px" }}>
                    ...
                    <div key={gi} style={{
                      ...
                      {assigned ? (
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          <button onClick={() => onAssign(key, null)}
                            style={{ fontSize: 10, padding: "1px 7px", borderRadius: 4, border: "1px solid #d1d5db", backgroundColor: "white", color: "#6b7280", cursor: "pointer" }}>
                            ✕ Desassignar
                          </button>
                        </div>
                      ) : (
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          <button onClick={() => onAssign(key, "PROF. DE GUÀRDIA")}
                          ...
                          <button onClick={() => onAssign(key, "NO_CAL_COBRIR")}
              ...
              <div style={{ flex: "2 1 260px" }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: "#166534", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 8px" }}>
                  Professors disponibles
                </p>
                {covers.freed.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    <span style={{ fontSize: 10, color: "#0f766e", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>🟢 Alliberats per la sortida</span>
                    <p style={{ fontSize: 10, color: "#9ca3af", margin: "1px 0 5px" }}>Tenien classe amb alumnes que han marxat</p>
                    <TeacherButtons teachers={covers.freed} time={time} gaps={gaps} assignments={assignments} assignedHere={assignedHere} onAssign={onAssign} colorBg="#ccfbf1" colorFg="#0f766e" colorBorder="#99f6e4" />
                  </div>
                )}
                {covers.guardL?.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    <span style={{ fontSize: 10, color: "#0369a1", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>📖 Guàrdia de lectura</span>
                    <p style={{ fontSize: 10, color: "#9ca3af", margin: "1px 0 5px" }}>Guàrdia assignada durant la lectura</p>
                    <TeacherButtons teachers={covers.guardL} time={time} gaps={gaps} assignments={assignments} assignedHere={assignedHere} onAssign={onAssign} colorBg="#e0f2fe" colorFg="#0369a1" colorBorder="#7dd3fc" />
                  </div>
                )}
                {covers.guardA.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    <span style={{ fontSize: 10, color: "#854d0e", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>🟡 Guàrdia A</span>
                    <p style={{ fontSize: 10, color: "#9ca3af", margin: "1px 0 5px" }}>Guàrdia assignada (A)</p>
                    <TeacherButtons teachers={covers.guardA} time={time} gaps={gaps} assignments={assignments} assignedHere={assignedHere} onAssign={onAssign} colorBg="#fef9c3" colorFg="#854d0e" colorBorder="#fde68a" />
                  </div>
                )}
                {covers.guardB.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    <span style={{ fontSize: 10, color: "#92400e", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>🟠 Guàrdia B</span>
                    <p style={{ fontSize: 10, color: "#9ca3af", margin: "1px 0 5px" }}>Guàrdia assignada (B)</p>
                    <TeacherButtons teachers={covers.guardB} time={time} gaps={gaps} assignments={assignments} assignedHere={assignedHere} onAssign={onAssign} colorBg="#ffedd5" colorFg="#92400e" colorBorder="#fed7aa" />
                  </div>
                )}
                {covers.guardE?.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    <span style={{ fontSize: 10, color: "#7c3aed", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>🎵 Guàrdia Pati Especial</span>
                    <p style={{ fontSize: 10, color: "#9ca3af", margin: "1px 0 5px" }}>Música / Biblioteca</p>
                    <TeacherButtons teachers={covers.guardE} time={time} gaps={gaps} assignments={assignments} assignedHere={assignedHere} onAssign={onAssign} colorBg="#ede9fe" colorFg="#7c3aed" colorBorder="#c4b5fd" />
                  </div>
                )}
                {covers.free.length > 0 && (
                  <div>
                    <button onClick={() => setShowFreeSlots((p) => ({ ...p, [time]: !p[time] }))}
                      style={{ fontSize: 10, fontWeight: 600, color: "#6b7280", background: "none", border: "1px solid #e5e7eb", borderRadius: 6, padding: "3px 10px", cursor: "pointer", marginBottom: showFree ? 6 : 0 }}>
                      {showFree ? "▲ Amagar" : "▼ Mostrar"} hores lliures ({covers.free.length})
                    </button>
                    {showFree && (
                      <>
                        <p style={{ fontSize: 10, color: "#9ca3af", margin: "4px 0 5px" }}>No tenen classe ni guàrdia programada</p>
                        <TeacherButtons teachers={covers.free} time={time} gaps={gaps} assignments={assignments} assignedHere={assignedHere} onAssign={onAssign} colorBg="#f3f4f6" colorFg="#4b5563" colorBorder="#d1d5db" />
                      </>
                    )}
                  </div>
                )}
                {totalAvail === 0 && covers.free.length === 0 && (
                  <span style={{ fontSize: 12, color: "#991b1b", fontStyle: "italic", fontWeight: 600 }}>⚠ Cap professor disponible</span>
                )}
              </div>
            </div>
          </div>
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
      {teachers.map((n) => {
        ...
        return (
          <button key={n} onClick={() => !isUsedElsewhere && handleClick(n)} disabled={isUsedElsewhere}
            title={isAssignedHere ? "Clic per desassignar" : isUsedElsewhere ? "Ja assignat a un altre grup" : "Clic per assignar"}
            style={{
              fontSize: 11, padding: "3px 9px", borderRadius: 5,
              cursor: isUsedElsewhere ? "not-allowed" : "pointer",
              fontWeight: isAssignedHere ? 700 : 500,
              border: `1.5px solid ${isAssignedHere ? "#166534" : isUsedElsewhere ? "#e5e7eb" : colorBorder}`,
              backgroundColor: isAssignedHere ? "#166534" : isUsedElsewhere ? "#f9fafb" : colorBg,
              color: isAssignedHere ? "white" : isUsedElsewhere ? "#d1d5db" : colorFg,
              transition: "all 0.1s",
            }}>
            {isAssignedHere ? "✓ " : ""}{n}
          </button>
...
function RankingCard({ r, i, trip, confirmed, onToggleConfirm, teacherFranjaMap, onAssignFranja }) {
  ...
  return (
    <div style={{
      ...
      border: isConfirmed ? "2px solid #166534" : i === 0 ? "2px solid #e8451e" : "1px solid #e5e7eb",
      animation: "fadeUp 0.2s ease", transition: "all 0.2s",
    }}>
      <div style={{ width: 30, textAlign: "center", flexShrink: 0, paddingTop: 2 }}>
        {medal ? <span style={{ fontSize: 20 }}>{medal}</span> : (
          <span style={{ fontSize: 12, fontWeight: 700, color: "#9ca3af", fontFamily: "monospace" }}>#{i + 1}</span>
        )}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 8 }}>
          ...
          {r.tripClasses.length > 0 && (
            <Pill color="navy">📚 {r.coveredSlots === Math.floor(r.coveredSlots) ? r.coveredSlots : `${r.coveredSlots}`}/{r.totalRange} h. amb el grup</Pill>
          )}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
          {r.inRange.map((slot, j) => {
            ...
            return (
              <span key={j} title={`${SLOT_LABEL[slot.time] || slot.time}: ${label}${slot.group ? " · " + slot.group : ""}`}
                style={{ fontSize: 10, padding: "2px 7px", borderRadius: 4, backgroundColor: bg, color: fg, fontWeight: 500 }}>
                <span style={{ opacity: 0.5, marginRight: 2, fontSize: 9 }}>{slot.time.split("-")[0]}</span>
                {prefix}{sl}
                {slot.group && slot.type !== "free" && <span style={{ opacity: 0.55 }}> · {slot.group}</span>}
              </span>
            );
          })}
        </div>
        {r.freeCount > 0 && (
          <p style={{ fontSize: 10, color: "#9ca3af", margin: "5px 0 0", fontStyle: "italic" }}>
            ◯ Hora lliure = cap activitat programada (ni classe, ni guàrdia, ni reunió)
          </p>
        )}
      </div>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, flexShrink: 0 }}>
        <div style={{ textAlign: "center" }}>
          <span style={{ display: "block", fontSize: 26, fontWeight: 800, color: isConfirmed ? "#166534" : i === 0 ? "#e8451e" : "#1a2744", lineHeight: 1, fontFamily: "monospace" }}>
            {r.score}
          </span>
          <span style={{ display: "block", fontSize: 9, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.07em", marginTop: 2 }}>pts</span>
        </div>
        <button onClick={() => { if (multiTorn) return; onToggleConfirm(r.name); }}
          style={{
            ...
            whiteSpace: "nowrap", display: multiTorn ? "none" : "block",
          }}>
        ...
        {multiTorn && (
          <div style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 90 }}>
            ...
            {isConfirmed && (
              <button onClick={() => { onToggleConfirm(r.name); onAssignFranja(r.name, []); }}
                style={{ padding: "3px 8px", borderRadius: 6, fontSize: 10, fontWeight: 600, cursor: "pointer", border: "1px solid #e5e7eb", backgroundColor: "transparent", color: "#9ca3af" }}>
                ✕ Treure
              </button>
            )}
          </div>
...
async function iEducaScraperFn() {
  ...
  if (!teacherSelect) {
    setStatus('❌ No trobat selector de professors'); removeUI(); return;
  }
  ...
  function isMeeting(full, subject) {
    return (
      full.includes('REUNIÓ') || full.includes('REUNIO') ||
      full.includes('EQUIP') || full.includes('CLAUSTRE') ||
      full.includes('DEPARTAMENT') || full.includes('DEP') ||
      /\bR\./.test(full) ||
      full.includes('DIRECCIO') || full.includes('DIRECCIÓ') ||
      full.includes('COORD') || full.includes('CONSELL') ||
      full.includes('PEDAGÒGIC') || full.includes('PEDAGOGIC') ||
      full.includes('STEAM') || full.includes('DIGITAL') ||
  ...
  function parseCell(cell, time) {
    ...
    if (full.includes('GUÀRDIA') || full.includes('GUARDIA'))
      return { time, subject: cleanTeacherName(rawText.join(' ')), group: '', room: '', type: 'guard' };
  ...
  function parseHTML(html, teacherName) {
    ...
    for (let ri = headerIdx + 1; ri < rows.length; ri++) {
      ...
      for (let di = 0; di < 5; di++) {
        schedule[DAYS[di]].push(parseCell(cells[di + 1], timeSlot));
      }
    }
...
export default function SortidesApp() {
  ...
  const processFiles = useCallback(async (files) => {
    ...
    for (const file of jsons) {
      ...
      try {
        ...
        if (!arr[0]?.name || !arr[0]?.schedule) throw new Error("Format no reconegut");
        mergeTeachers(arr.map(normalizeTeacher));
      } catch (e) { setErrors((p) => [...p, `Error JSON "${file.name}": ${e.message}`]); }
    }
    for (const file of pdfs) {
      ...
      } catch (e) { setErrors((p) => [...p, `Error PDF "${file.name}": ${e.message}`]); }
    }
    for (const file of imgs) {
      ...
      try {
        ...
        mergeTeachers([data]);
      } catch (e) { setErrors((p) => [...p, `Error imatge "${file.name}": ${e.message}`]); }
    }
  ...
  const handleCompute = () => {
    ...
    const tripForRanking = {
      day: trip.day, startSlot: globalStart, endSlot: globalEnd,
      selectedGroups: allGroups, excludedSubs: allExcludedSubs,
      halfGroups: allHalfGroups, subject: trip.subject,
    };
    const r = teachers.map((t) => analyzeTeacher(t, tripForRanking));
    r.sort((a, b) => a.hoursShort !== b.hoursShort ? a.hoursShort - b.hoursShort : b.score - a.score);
    setRanking(r); setConfirmed(new Set()); setAssignments({}); setTeacherFranjaMap({});
    setStep("ranking");
  };
  ...
  const buildExportData = () => {
    ...
    const acompanyants = trip.franges.map((f) => {
      const assignedProfs = [...confirmed].filter(name => (teacherFranjaMap[name] || []).includes(f.id));
      const profsToShow = assignedProfs.length > 0 ? assignedProfs : [...confirmed];
      return { hora: `${f.startSlot.split("-")[0]} – ${f.endSlot.split("-")[1]}`, grups: compactaGrups(f.selectedGroups), professors: profsToShow, responsables: "" };
    });

    return {
      event: { title: trip.titol || allGroups.join(", "), subtitle: trip.lloc || "", date: trip.data || DAY_LABELS[trip.day] || trip.day },
      acompanyants,
  ...
  const generateDocHTML = (data) => {
    ...
    acomp.forEach((a) => {
      ...
      acompRows += `<tr><td class="cell-hora">${esc(a.hora)}</td><td class="cell-grups">${esc(a.grups)}</td><td class="cell-profs"><ul class="prof-list">${profs}</ul></td><td class="cell-resp">${esc(nc(a.responsables))}</td></tr>`;
    });
    let subsRows = "";
    franges.forEach((f) => {
      ...
      f.cobertures.forEach((c) => {
        ...
        const sub = c.substitut ? `<span class="substitut">${esc(subNom)}</span>` : `<span class="uncovered">⚠ Sense cobrir</span>`;
        const titularNom = na(c.professorOriginal) || esc(c.professorOriginal);
        subsRows += `<tr class="cob-row"><td class="col-hora">${esc(f.hora)}</td><td class="col-ass">${esc(c.assignatura)}</td><td class="col-grp">${esc(c.grup)}</td><td class="col-aul">${esc(c.aula)}</td><td class="col-ori">${esc(titularNom)}</td><td>${sub}</td></tr>`;
      });
    });
    const now = new Date().toLocaleString("ca-ES");
...
<div class="doc-footer">Document generat el ${now}</div>
  ...
  const handleAssign = (key, name) => {
    setAssignments((prev) => { const n = { ...prev }; if (name === null) delete n[key]; else n[key] = name; return n; });
  };
        ...
        <div style={{ maxWidth: 940, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 62 }}>
          ...
          <nav style={{ display: "flex", gap: 2 }}>
            {[["upload", "Professors"], ["trip", "Sortida"], ["ranking", "Ranking & Cobertura"]].map(([s, label], idx) => {
              ...
              return (
                <button key={s} disabled={locked} onClick={() => !locked && setStep(s)}
                  style={{
                    ...
                    backgroundColor: step === s ? "rgba(255,255,255,0.12)" : "transparent",
                  }}>
                  <span style={{ width: 18, height: 18, borderRadius: "50%", border: `1.5px solid ${step === s ? "white" : "rgba(255,255,255,0.4)"}`, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700 }}>
                    {idx + 1}
                  </span>
                  {label}
                </button>
          ...
          <div>
            ...
            {savedAt && teachers.length > 0 && (
              <div style={{ background: "#f0fdf4", border: "1.5px solid #86efac", borderRadius: 10, padding: "12px 16px", marginBottom: 14, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: "#166534", margin: 0 }}>Horaris carregats des de la memòria del navegador</p>
                  <p style={{ fontSize: 12, color: "#4b7c59", margin: "2px 0 0" }}>{teachers.length} professors guardats · Actualitzat el {savedAt}</p>
                </div>
                <button
                  onClick={() => {
                    if (window.confirm("Vols esborrar els horaris guardats? Hauràs de tornar a pujar el JSON.")) {
                      ...
                      setTeachers([]); setSavedAt(null);
                    }
                  }}
                  style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid #86efac", background: "white", color: "#c0392b", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                  🗑 Esborrar
                </button>
              </div>
            )}
            ...
            <p style={{ fontSize: 12, color: "#6b7280", margin: "0 0 18px", lineHeight: 1.45 }}>
              Entra a iEduca a la pàgina d'horaris i clica al botó a favorits per a extreure els horaris dels professors.
            </p>
            <p style={{ fontSize: 14, color: "#6b7280", margin: "0 0 22px" }}>
              Puja el <strong style={{ color: "#374151" }}>JSON del scraper iEduca</strong>, un PDF o imatges individuals.
            </p>
            ...
            <div
              ...
              {processing ? (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                  <Spinner />
                  <p style={{ fontSize: 14, color: "#374151", margin: 0 }}>Processant <strong>{processingName}</strong></p>
                </div>
              ) : (
                <>
                  <div style={{ fontSize: 28, marginBottom: 8, color: "#d1d5db" }}>↑</div>
                  <p style={{ fontSize: 15, color: "#374151", margin: "0 0 12px" }}>
                    Arrossega aquí o <span style={{ color: "#e8451e", fontWeight: 600, textDecoration: "underline" }}>fes clic</span>
                  </p>
                  <div style={{ display: "flex", justifyContent: "center", gap: 8, flexWrap: "wrap" }}>
                    {[{ icon: "📋", label: "JSON scraper", sub: "Recomanat" }, { icon: "📄", label: "PDF tots els horaris", sub: "Via IA" }, { icon: "🖼️", label: "Imatges JPG/PNG", sub: "Un per un" }].map(({ icon, label, sub }) => (
                      <div key={label} style={{ padding: "8px 14px", borderRadius: 8, border: "1px solid #e5e7eb", backgroundColor: "#fafafa", textAlign: "center", minWidth: 120 }}>
            ...
            {errors.length > 0 && (
              <div style={{ marginTop: 8, padding: "10px 14px", backgroundColor: "#fef2f2", borderRadius: 8, border: "1px solid #fecaca" }}>
                {errors.map((e, i) => <p key={i} style={{ fontSize: 12, color: "#991b1b", margin: "1px 0" }}>⚠ {e}</p>)}
              </div>
            )}
            ...
            {teachers.length > 0 && (
              <div style={{ marginTop: 26 }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 10px" }}>
                  {teachers.length} professors · {totalClasses} classes setmanals
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 18 }}>
                  {teachers.map((t) => {
                    const nC = Object.values(t.schedule).flat().filter((s) => s.type === "class").length;
                    const grps = [...new Set(Object.values(t.schedule).flat().filter((s) => s.type === "class").map((s) => s.group).filter(Boolean))];
                    return (
                      <div key={t.name} style={{ display: "flex", alignItems: "center", gap: 12, backgroundColor: "white", borderRadius: 10, padding: "10px 14px", border: "1px solid #e5e7eb" }}>
                        <Avatar name={t.name} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 14, fontWeight: 600, color: "#1a2744", margin: "0 0 1px" }}>{t.name}</p>
                          <p style={{ fontSize: 11, color: "#9ca3af", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {nC} classes · {grps.slice(0, 5).join(", ")}{grps.length > 5 ? "…" : ""}
                          </p>
                        </div>
                        <button onClick={() => setTeachers((p) => p.filter((x) => x.name !== t.name))}
                          style={{ background: "none", border: "none", color: "#d1d5db", cursor: "pointer", fontSize: 16, padding: "2px 5px" }}>✕</button>
                      </div>
                    );
                  })}
                </div>
                <button onClick={() => setStep("trip")}
                  style={{ backgroundColor: "#e8451e", color: "white", border: "none", borderRadius: 9, padding: "12px 26px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
          ...
          <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {[
                  ...
                  <div key={key} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <input type="text" value={trip[key]}
                      onChange={(e) => setTrip((t) => ({ ...t, [key]: e.target.value }))}
                      placeholder={placeholder}
                      style={{ flex: 1, padding: "7px 11px", borderRadius: 7, border: "1.5px solid #e5e7eb", fontSize: 13, color: "#111827", backgroundColor: "#ffffff" }} />
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card title="Dia de la sortida">
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                {DAYS.map((day) => (
                  <button key={day} onClick={() => setTrip((t) => ({ ...t, day }))}
                    style={{
                      padding: "9px 18px", borderRadius: 8,
                      border: `1.5px solid ${trip.day === day ? "#1a2744" : "#e5e7eb"}`,
              ...
              return (
                <div key={franja.id} style={{ border: "1.5px solid #e5e7eb", borderRadius: 12, padding: "16px", marginBottom: 14, background: "#f9fafb", position: "relative" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: "#1a2744", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                      Torn {fi + 1}
                      {trip.franges.length > 1 && <span style={{ fontWeight: 400, color: "#6b7280", marginLeft: 6 }}>— {franja.startSlot.split("-")[0]}–{franja.endSlot.split("-")[1]}</span>}
                    </span>
                    {trip.franges.length > 1 && (
                      <button onClick={() => setTrip((t) => ({ ...t, franges: t.franges.filter((f) => f.id !== franja.id) }))}
                        style={{ background: "none", border: "none", color: "#c0392b", cursor: "pointer", fontSize: 13, fontWeight: 600, padding: "2px 6px" }}>
                        ✕ Eliminar torn
                      </button>
                    )}
                  </div>
                  <Card title="Horari del torn" hint="Inici i final d'aquest torn.">
                    <TimeRangePicker startSlot={franja.startSlot} endSlot={franja.endSlot} onChange={(s, e) => updateFranja({ startSlot: s, endSlot: e })} />
                  </Card>
                  <Card title="Professors acompanyants necessaris" hint="Quants professors han d'anar en aquest torn?">
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <button onClick={() => updateFranja({ neededCount: Math.max(1, (franja.neededCount || 2) - 1) })}
                        style={{ width: 36, height: 36, borderRadius: 8, border: "1.5px solid #e5e7eb", backgroundColor: "white", fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#374151" }}>−</button>
                      ...
                      <button onClick={() => updateFranja({ neededCount: (franja.neededCount || 2) + 1 })}
                        style={{ width: 36, height: 36, borderRadius: 8, border: "1.5px solid #e5e7eb", backgroundColor: "white", fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#374151" }}>+</button>
                      <span style={{ fontSize: 13, color: "#6b7280" }}>professor{(franja.neededCount || 2) > 1 ? "s" : ""} acompanyant{(franja.neededCount || 2) > 1 ? "s" : ""}</span>
                    </div>
                  </Card>
                  <Card title="Grups d'alumnes" hint="Selecciona els grups d'aquest torn.">
                    <GroupSelector
            ...
            {trip.franges.length < 6 && (
              <button onClick={() => setTrip((t) => ({ ...t, franges: [...t.franges, newFranja()] }))}
                style={{ width: "100%", padding: "11px", border: "1.5px dashed #1a2744", borderRadius: 10, background: "white", color: "#1a2744", fontSize: 13, fontWeight: 600, cursor: "pointer", marginBottom: 14 }}>
                + Afegeix un nou torn
              </button>
            )}

            <Card title="Matèria relacionada" hint="Opcional. Puntua extra als professors d'aquesta matèria.">
              <SubjectSelector subjects={subjects} selected={trip.subject} onChange={(v) => setTrip((t) => ({ ...t, subject: v }))} />
            </Card>

            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
              <button onClick={() => setStep("upload")}
                style={{ backgroundColor: "white", color: "#374151", border: "1px solid #e5e7eb", borderRadius: 9, padding: "11px 20px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
                ← Enrere
              </button>
              <button onClick={handleCompute}
                style={{ backgroundColor: "#e8451e", color: "white", border: "none", borderRadius: 9, padding: "11px 26px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
                Calcular ranking →
              </button>
            </div>
          </div>
        ...
        {step === "ranking" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18, gap: 12, flexWrap: "wrap" }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: "#6b7280", margin: 0 }}>
                  ...
                  {trip.franges.map((f, fi) => (
                    <span key={f.id}>
                      {fi > 0 && " · "}
                      <strong style={{ color: "#374151" }}>{f.startSlot.split("-")[0]}–{f.endSlot.split("-")[1]}</strong>
                      {f.selectedGroups.length > 0 && <span style={{ color: "#374151" }}> ({f.selectedGroups.join(", ")})</span>}
                    </span>
                  ))}
              ...
              <button onClick={() => setStep("trip")}
                style={{ backgroundColor: "white", color: "#374151", border: "1px solid #e5e7eb", borderRadius: 9, padding: "8px 14px", fontSize: 12, fontWeight: 500, cursor: "pointer", flexShrink: 0 }}>
                ← Canviar
              </button>
            </div>
            ...
            <div style={{ backgroundColor: "#1a2744", borderRadius: 12, padding: "14px 20px", marginBottom: 14 }}>
              ...
              {confirmed.size === 0 ? (
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", margin: 0, fontStyle: "italic" }}>
                  {trip.franges.length > 1 ? "Fes clic a \"+ Torn 1\" o \"+ Torn 2\" per assignar professors a cada torn" : "Fes clic a \"+ Confirmar\" per marcar els professors que van de sortida"}
                </p>
              ) : (
                <>
                  {trip.franges.length > 1 ? (
                          ...
                          <div key={franja.id} style={{ background: "rgba(255,255,255,0.07)", borderRadius: 8, padding: "8px 12px" }}>
                            ...
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                              {profsInFranja.length === 0
                                ...
                                : profsInFranja.map(n => (
                                  <span key={n} style={{ fontSize: 12, padding: "3px 10px", borderRadius: 99, backgroundColor: "#166534", color: "white", fontWeight: 500 }}>✓ {n}</span>
                                ))
                              }
                            </div>
                          </div>
                  ...
                  <div style={{ textAlign: "right", marginTop: 8 }}>
                    <span style={{ fontSize: 13, color: remaining === 0 ? "#4ade80" : "#e8451e", fontWeight: 700 }}>
                      {confirmed.size} confirmat{confirmed.size !== 1 ? "s" : ""}{remaining > 0 ? ` · Falten ${remaining}` : " · ✓ Cobert"}
                    </span>
                  </div>
                </>
              )}
            ...
            {confirmed.size > 0 && (
              <div style={{ marginBottom: 16 }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.08em", margin: "0 0 8px" }}>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
                  ...
                  <button
                    onClick={() => {
                      ...
                      const acomp = data.acompanyants || [];
                      const franges = data.franges || [];
                      const LILA = "FF674EA7", FOSC = "FF20124D", BLAU_CL1 = "FFD9E1F2", BLAU_CL2 = "FFF0F3FA", BLAU_CL3 = "FFCFE2F3", BLANC = "FFFFFFFF", BLAU_TEXT = "FF0000FF";
                      const nc = (n) => { if (!n) return ""; const p = n.split(",").map(s=>s.trim()); return p.length>=2?`${p[1]} ${p[0]}`:n; };
                      const na = (n) => { if (!n) return ""; const c=nc(n).trim().split(/\s+/); return c.length<2?nc(n):`${c[0]} ${c[1][0]}.`; };
                      ...
                      const richSub = (subNom, resta) => ({
                        v: `${subNom}${resta}`, t: "s",
                        s: { font: { bold: true, sz: 12, color: { rgb: "FFCC0000" } }, fill: undefined, alignment: { wrapText: true, vertical: "center", horizontal: "left" }, border: { top:{style:"thin",color:{rgb:"FFCCCCCC"}}, bottom:{style:"thin",color:{rgb:"FFCCCCCC"}}, left:{style:"thin",color:{rgb:"FFCCCCCC"}}, right:{style:"thin",color:{rgb:"FFCCCCCC"}} } },
                        r: [{ t: subNom, s: { font: { bold: true, sz: 12, color: { rgb: "FFCC0000" } } } }, { t: resta, s: { font: { bold: false, sz: 12, color: { rgb: "FF000000" } } } }]
                      });
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
                {FILTERS.map((f) => {
                  ...
                  return (
                    <button key={f.id} onClick={() => toggleFilter(f.id)} title={f.desc}
                      style={{ padding: "6px 12px", borderRadius: 99, border: `1.5px solid ${a ? "#1a2744" : "#e5e7eb"}`, backgroundColor: a ? "#1a2744" : "white", color: a ? "white" : "#4b5563", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>
                      {a && "✓ "}{f.label}
                    </button>
                  );
                })}
                {activeFilters.size > 0 && (
                  <button onClick={() => setActiveFilters(new Set())}
                    style={{ padding: "6px 12px", borderRadius: 99, border: "1.5px solid #fecaca", backgroundColor: "#fef2f2", color: "#991b1b", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>
                    ✕ Netejar filtres
                  </button>
                )}
              </div>
              <p style={{ fontSize: 11, color: "#9ca3af", margin: 0 }}>
                Ordenat per: <strong style={{ color: "#4b5563" }}>hores cobertes del grup ↓</strong> · després per puntuació.
              </p>
            </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "3px 14px", fontSize: 11, color: "#6b7280", marginBottom: 10, padding: "10px 14px", backgroundColor: "white", borderRadius: 8, border: "1px solid #e5e7eb" }}>
              ...
              {[
                { bg: "#dcfce7", c: "#166534", t: "Classe amb el grup de la sortida" },
                { bg: "#fee2e2", c: "#991b1b", t: "Classe amb grup que es queda (genera forat)" },
                { bg: "#fff7ed", c: "#9a3412", t: "Mig grup que es queda (½ forat)" },
                { bg: "#fef9c3", c: "#854d0e", t: "Guàrdia assignada" },
                { bg: "#f3e8ff", c: "#6b21a8", t: "Reunió / tutoria tècnica" },
                { bg: "#f3f4f6", c: "#9ca3af", t: "Hora lliure (sense classe programada)" },
              ].map(({ bg, c, t }) => (
                <span key={t}>
                  <span style={{ display: "inline-block", width: 10, height: 10, borderRadius: 2, backgroundColor: bg, border: `1px solid ${c}22`, marginRight: 4, verticalAlign: "middle" }} />
                  {t}
                </span>
              ))}
            </div>
            ...
            <p style={{ fontSize: 12, color: "#9ca3af", margin: "0 0 8px" }}>
              ...
              {activeFilters.size > 0 && <> · <span style={{ color: "#e8451e" }}>{activeFilters.size} filtre{activeFilters.size > 1 ? "s" : ""} actiu{activeFilters.size > 1 ? "s" : ""}</span></>}
            </p>

            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {sortedRanking.length === 0 ? (
                <div style={{ padding: "32px", textAlign: "center", backgroundColor: "white", borderRadius: 12, border: "1px solid #e5e7eb" }}>
                  <p style={{ fontSize: 14, color: "#6b7280", margin: "0 0 10px" }}>Cap professor compleix els filtres actius.</p>
                  <button onClick={() => setActiveFilters(new Set())}
                    style={{ padding: "7px 14px", borderRadius: 8, border: "1px solid #e5e7eb", backgroundColor: "white", color: "#374151", fontSize: 13, cursor: "pointer" }}>
                    Treure filtres
                  </button>
                </div>
              ) : (
                sortedRanking.map((r, i) => (
                  <RankingCard key={r.name} r={r} i={i} trip={trip} confirmed={confirmed} onToggleConfirm={toggleConfirm}
                    teacherFranjaMap={teacherFranjaMap}
